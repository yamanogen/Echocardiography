...
int main(void)
{
	...
	DcmElement *element;
	Uint8 *pixeldata;
	int data_number = 12; int data_index = 2;
	int blood_outlier = 0;
	bool Blood_cluster_(false);
	bool Blood_cluster_before_(false);
	bool Blood_cluster_after_(false);
	bool Valve_Location_(false);
	int negaFromCloud_size = 0;
	int outlierSize = 800;


	if (dcmfile.loadFile(DCMfilenameTest1_1).bad()) {
	...
	vector<vector<PointCloud<PointXYZRGB>::Ptr>> cloud_clustered_before, cloud_clustered_before_gathered, cloud_clustered_after;
	cloud_clustered_before.resize(cloud.size()); cloud_clustered_before_gathered.resize(cloud.size()); cloud_clustered_after.resize(cloud.size());
	...
	if (Blood_) {
		...
		if (Blood_cluster_before_) {
			//blood region clustering
			for (size_t f = 0; f < cloud.size(); f++) {
				vector<PointCloud<PointXYZRGB>::Ptr> cloud_clustered;
				int count = 1;
				//statisticalOutlierRemoval(cloud_Blood[f], cloud_Blood[f], OutlierK, 0.5f);
				cout << "cloud_Blood[" << f << "] found: " << cloud_Blood[f]->size() << endl;
				euclideanClustering(cloud_Blood[f], 8.0f, 10, 10000, cloud_clustered);

				vector<vector<PointCloud<PointXYZRGB>::Ptr>> temp;
				//temp.resize(cloud_clustered.size());

				while (count > 0) {
					temp.resize(cloud_clustered.size());
					count = 0;
					for (size_t n = 0; n < cloud_clustered.size(); n++) {
						//printf("n:%d\n", n);
						/*-----------------------------------------------------*/
						if (cloud_clustered[n]->size() > outlierSize) {
							/*-----------------------------------------------------*/

							statisticalOutlierRemoval(cloud_clustered[n], cloud_clustered[n], OutlierK, 0.5f);
							euclideanClustering(cloud_clustered[n], 8.0f, 10, 10000, temp[n]);
							count++;
						}
						else if (cloud_clustered[n]->size() != 0) {
							temp[n].push_back(cloud_clustered[n]);
						}
					}

					cloud_clustered.clear();
					for (size_t n = 0; n < temp.size(); n++) {
						for (size_t m = 0; m < temp[n].size(); m++) {
							//printf("m:%d\n", m);
							if (temp[n][m]->size() != 0) cloud_clustered.push_back(temp[n][m]);
						}
					}
					temp.clear();
				...
				//last gather cloud_clustered_before
				for (size_t n = 0; n < cloud_clustered.size(); n++) {
					cloud_clustered_before[f].push_back(cloud_clustered[n]);
				}
	...
	while (!viewer->wasStopped()) {
		...
		if (Blood_) {
			...
			if (Blood_cluster_) {
				...
				cout << "cloud_Blood[" << current << "] found: " << cloud_Blood[current]->size() << endl;
			}
		}
		if (Valve_Location_) {
