...
int main(void)
{
	...
	DcmElement *element;
	Uint8 *pixeldata;
	if (dcmfile.loadFile(DCMfilenameFail1).bad()) {
	...
	*/
	...
	//ダウンサンプリング
	//pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud1_filtered(new pcl::PointCloud<pcl::PointXYZRGB>);
	double filterSizeX = PixelSpacingX * D;
	double filterSizeY = PixelSpacingY * D;
	double filterSizeZ = PixelSpacingZ * D;

	DownSampling(cloud1, cloud1, filterSizeX, filterSizeY, filterSizeZ);
