...
int main(void)
{
	...
	for (int i = 0; i < 9; i++) {//more than 1
		...
		cloudMakeLeft(cloud_elem, pixeldata, i + 1);
	...
	for (int n = 0; n < 2; n++) {
		for (size_t i = 0; i < cloud.size(); i++) {
			statisticalOutlierRemoval(cloud[i], cloud[i], OutlierK, 1);
		}
	}
	...
	// DownSampling
	for (size_t i = 0; i < cloud.size(); i++) {
		uniformSampling(cloud_keypoints[i], D, cloud_keypoints[i]);
	}
	...
	while (!viewer->wasStopped()) {
		...
		if (show_correspondences_) {
			for (size_t i = 0; i < cloud_corrs[current].size(); i += 3) {
				...
				if (euclideanDistance(center_point, cloudA_point) < 185) {
					...
					if ((cloudB_point.x - cloudA_point.x) >= 0 && (cloudB_point.y - cloudA_point.y) >= 0 && (cloudB_point.z - cloudA_point.z) >= 0) viewer->addLine<PointXYZRGB, PointXYZRGB>(cloudA_point, cloudB_point, 0, 0, 1.0, ss_line.str());
					...
					else if ((cloudB_point.x - cloudA_point.x) < 0 && (cloudB_point.y - cloudA_point.y) < 0 && (cloudB_point.z - cloudA_point.z) < 0) viewer->addLine<PointXYZRGB, PointXYZRGB>(cloudA_point, cloudB_point, 1.0, 0, 0, ss_line.str());
					else;
