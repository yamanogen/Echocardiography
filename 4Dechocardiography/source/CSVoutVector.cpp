...
int main(void)
{
	...
	DcmElement *element;
	Uint8 *pixeldata;
	if (dcmfile.loadFile(DCMfilenameTest13).bad()) {
	...
	for (int i = 0; i < 11; i++) {//more than 1
	...
	//for (size_t i = 0; i < cloud.size(); i++) {
	//	if (i != (cloud.size() - 1)) CSVoutVector(cloud_keypoints[i], cloud_keypoints[i + 1], cloud_corrs[i], i);
	//	else CSVoutVector(cloud_keypoints[i], cloud_keypoints[0], cloud_corrs[i], i);
	//}

	//vector_ave csv out
	char filenameVector[256];
	sprintf_s(filenameVector, "C:\\Users\\guest1\\Desktop\\心臓データ\\20180114\\csv\\13\\ave.csv");
	ofstream save(filenameVector);
	for (size_t i = 0; i < cloud.size(); i++) {
		...
		for (size_t j = 0; j < cloud_corrs[i].size(); j += 1) {

			PointXYZRGB cloudP_point;
			PointXYZRGB cloudN_point;
			if (i != (cloud.size() - 1)) {
				cloudP_point = cloud_keypoints[i]->at(cloud_corrs[i][j].index_match);
				cloudN_point = cloud_keypoints[i + 1]->at(cloud_corrs[i][j].index_query);
			}
			else {
				cloudP_point = cloud_keypoints[i]->at(cloud_corrs[i][j].index_match);
				cloudN_point = cloud_keypoints[0]->at(cloud_corrs[i][j].index_query);
			}
		...
		vector_ave.x = vector_ave.x / cloud_corrs[i].size();
		vector_ave.y = vector_ave.y / cloud_corrs[i].size();
		vector_ave.z = vector_ave.z / cloud_corrs[i].size();
